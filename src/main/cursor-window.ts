import { BrowserIndow, Display } from 'electron';
import * as path from 'path';

let cursorWindow: BrowserIndow | null = null;

function getPreloadPath(): string {
  return path.join(__dirname, '../preload/index.js');
}

export function createCursorOverlayWindow(display: Display): BrowserIndow {
  const { x, y, width, height } = display.bounds;

  cursorWindow = new BrowserIndow({
    x, y, width, height,
